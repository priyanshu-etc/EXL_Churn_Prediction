...
# Load dataset
df = pd.read_csv('engineered_exl_credit_card_churn_data.csv')
os.makedirs('engineered_EDA', exist_ok=True)
...
def plot_distribution(column, bins=30, kde=True, color='skyblue'):
    ...
    plt.tight_layout()
    plt.savefig(f'engineered_EDA/{column.lower()}_distribution.png')
    plt.close()
...
if 'Tenure' in df.columns:
    ...
    plt.ylabel('Count')
    plt.tight_layout()
    plt.savefig('engineered_EDA/tenure_distribution.png')
    plt.close()
...
if 'Gender' in df.columns:
    ...
    plt.tight_layout()
    plt.savefig('engineered_EDA/gender_distribution.png')
    plt.close()
...
plt.tight_layout()
plt.savefig('engineered_EDA/churn_count.png')
plt.close()
...
plt.tight_layout()
plt.savefig('engineered_EDA/correlation_matrix.png')
plt.close()
...
for col in ['Age', 'Balance', 'EstimatedSalary']:
    if col in df.columns:
        ...
        plt.tight_layout()
        plt.savefig(f'engineered_EDA/{col.lower()}_boxplot.png')
        plt.close()
...
plt.ylabel('Age')
plt.tight_layout()
plt.savefig('engineered_EDA/age_vs_churn.png')
plt.close()
...
if all(col in df.columns for col in selected_cols):
    ...
    plt.savefig('engineered_EDA/pairplot.png')
    plt.close()
